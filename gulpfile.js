...
const gulp         = require('gulp'), // Подключаем Gulp
			...
			neat         = require('node-neat').includePaths, // Подключаем Neat
...
gulp.task('styles', function() {
	...
	return gulp.src('src/sass/**/*.sass') // Берем все файлы с расширением sass
		...
		.pipe(sass({
			...
			includePaths: neat
		})) // Преобразуем Sass/Bourbon/Neat в CSS посредством gulp-sass
...
gulp.task('sprite', function() {
    var spriteData =
        gulp.src('./src/images/icons/*.*') // путь, откуда берем картинки для спрайта
            .pipe(spritesmith({
                imgName: 'sprite.png',
                cssName: 'sprite.css',
                cssFormat: 'css',
    ...
    spriteData.css.pipe(gulp.dest('./src/images/sprite')); // путь, куда сохраняем стили
});
